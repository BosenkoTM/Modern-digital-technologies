...
```mermaid
flowchart TD
    A["Требуется<br/>сетевой обмен"] --> B{"Много подписчиков<br/>на события?"}

    B -->|Да| C["MQTT"]

    B -->|Нет| D{"Устройство предоставляет<br/>REST-подобные<br/>ресурсы?"}

    D -->|Да| E["CoAP"]

    D -->|Нет| F["HTTP / MQTT /<br/>специализированный<br/>протокол"]
```
