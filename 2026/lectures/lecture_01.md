...
```mermaid
flowchart TD
    A[Новая вычислительная задача] --> B{Критична задержка?}
    B -->|Да| C{Помещается на устройстве?}
    C -->|Да| D[Device computing]
    C -->|Нет| E[Edge computing]
    B -->|Нет| F{Нужны большие данные или тяжелая модель?}
    F -->|Да| G[Cloud / сервер]
    F -->|Нет| H[Edge или локальный сервер]
```
